...
function MakeUserMove() {
	...
	if(UserMove.from != SQUARES.NO_SQ && UserMove.to != SQUARES.NO_SQ) {
		...
		if(parsed != NOMOVE) {
			...
			PrintBoard();
			MoveGUIPiece(parsed);
			console.log('making ajax call to send user move')
			SendToPy(PrSq(UserMove.from) + PrSq(UserMove.to), recordedPosition)
		}
...
function AddGUIPiece(sq, pce) {
	...
	var pieceFileName = "/static/chessai/images/" + SideChar[PieceCol[pce]] + PceChar[pce].toUpperCase() + ".png";
...
function CheckResult() {
	if(GameBoard.fiftyMove >= 100) {
		...
		$("#GameStatus").after('<br id="game-status-break">') 
		return BOOL.TRUE;
	}
...
$('#SearchButton').click( function () {	
	if(GameController.side == 'w'){
		GameController.side = 'b'
	}
	else{
		GameController.side = 'w'
	}
	SendToPy('None', recordedPosition)
});

function StartSearch() {
	binary_move = parseInt(bits + MoveDec2Bin(moveTo) + MoveDec2Bin(moveFrom), 2);
	
	console.log('movefrom in SQ120 format ' + moveFrom)
	console.log('moveto in SQ120 format ' + moveTo)
	console.log('move encoded ' + binary_move);
	console.log('Computer Move: ' + PrMove(binary_move));

	MakeMove(binary_move);
	MoveGUIPiece(binary_move);
	CheckAndSet();
}
...
function ShowThinking(){
	...
	$('#EngineOutput').append('<button type="button" id="thinking-btn">Thinking</button>');
	$('#thinking-btn').prop('disabled', true)
	$('#thinking-btn').css('width', '480px')
}

function RemoveThinking(){
	$('#thinking-btn').remove();
	$('#EngineOutput').append('<button type="button" id="SearchButton">Switch Sides</button>');
	$('#EngineOutput').append('<button type="button" id="NewGameButton">New Game</button>');
...
function SendToPy(input, lastboard) {
	...
	$.ajax({
		...
		async: true,
		crossDomain: true,
		data: {move_sent_to_py: input, position: lastboard, 
			   ComputerColor: GameController.side},
		beforeSend: ShowThinking()
		}).done(function(response){
			console.log('success');
			recordedPosition = response['position'];
			moveFrom = SQ120(response['move_from']);
			moveTo = SQ120(response['move_to']);
			bits = response['bits'];
...
function MoveDec2Bin(dec){
	var num = (dec >>> 0).toString(2)
    return "0000000".substr(num.length) + num;
}
